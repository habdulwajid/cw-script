...
# Plugin status checker on bulk
#!/bin/bash

echo "Plugin status: hseo"
...
for root_path in "${roots[@]}"; do
    ...
    if timeout 60 wp plugin is-installed hseo --allow-root >/dev/null 2>&1; then
        if timeout 60 wp plugin is-active hseo --allow-root >/dev/null 2>&1; then
            status="ACTIVE"
